...
class UsersService {
  ...
  // project get by client and editor
  public async getProjectsByUserRole(user_id: number): Promise<any[]> {
    //projects where user is client
    const projectsAsClient = await DB('projects_task')
      .where('client_id', user_id)
      .select('*');


    //projects where user is editor
    const projectsAsEditor = await DB('projects_task')
    ...
    return {
      user_id,
      projects_as_client: projectsAsClient,
      projects_as_editor: projectsAsEditor,
    };
  }
}
