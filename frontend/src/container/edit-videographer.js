...
const EditVideoGrapher = () => {
    ...
    useEffect(() => {
        const fetchVideoGrapher = async () => {
            try {
                const user = getLoggedInUser();
                if (!user?.user_id) {
                    showErrorToast("Please log in to Edit VideoGrapher.");
                    navigate("/login");
                    return;
                }

                const response = await makeGetRequest(`videographers/${id}`);
                console.log("Video Grapher API Response:", response); // Debug log
                ...
                if (!editorUser && !profile) {
                    showErrorToast("VideoGrapher not found.");
    ...
    const handleDelete = async () => {
        if (!canEdit) {
            showErrorToast("You are not authorized to delete this videographer.");
            return;
        }

        showAlert({
            title: "Are you sure?",
            text: "Are you sure you want to delete this videographer? This action cannot be undone.",
            icon: "warning",
            confirmButton: {
                text: "Yes, Delete",
                backgroundColor: "#c0392b",
            ...
            onConfirm: async () => {
                try {
                    ...
                    showSuccessToast("Videographer deleted successfully!");
                    navigate("/videographerhomepage");
                } catch (error) {
                    console.error("Delete error:", error);
                    showErrorToast(error.response?.data?.message || "Failed to delete videographer.");
                }
            },
        });
    };
    ...
    if (loading) {
        return (
            <Layout>
                <div>Loading videographer data...</div>
            </Layout>
        );
    }
